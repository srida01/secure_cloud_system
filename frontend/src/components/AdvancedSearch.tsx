...
export default function AdvancedSearch({ onResultClick, onClose }: Props) {
  ...
  const search = async () => {
    if (!query.trim() && !mimeType && !dateFrom && !dateTo) {
  ...
  const clearAll = () => {
    setQuery(''); setMimeType(''); setDateFrom('');
    ...
    <div style={overlay} onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div style={modal}>
        ...
        <div style={{ padding: '20px 24px 0', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ fontWeight: 700, fontSize: 18, color: '#e2e8f0' }}>Advanced Search</div>
        ...
        {/* Results */}
        {results !== null && (
          <div style={{ borderTop: '1px solid #1e293b', maxHeight: 360, overflow: 'auto' }}>
            <div style={{ padding: '12px 24px', fontSize: 12, color: '#64748b', fontWeight: 600 }}>
            ...
            {results.folders?.map((f: any) => (
              ...
              >
                ...
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 13, fontWeight: 600, color: '#fbbf24' }}>{f.name}</div>
                  <div style={{ fontSize: 11, color: '#475569' }}>Folder · {new Date(f.createdAt).toLocaleDateString()}</div>
                </div>
              </div>
            ))}
...
const inputStyle: React.CSSProperties = {
  ...
  border: '1px solid #334155', color: '#e2e8f0', borderRadius: 6,
  fontSize: 13, boxSizing: 'border-box',
};
const primaryBtn: React.CSSProperties = {
  flex: 1, padding: '10px 20px', background: '#6366f1', color: '#fff',
...
const resultRow: React.CSSProperties = {
  ...
  padding: '10px 24px', cursor: 'pointer', borderBottom: '1px solid #0f172a',
  transition: 'background 0.1s',
};
const tagChip: React.CSSProperties = {
  fontSize: 10, background: '#334155', color: '#94a3b8',
